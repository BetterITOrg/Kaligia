...
void make_cv::run(settings &s){
 ...
 vector<double> bgl_start, bgl_end;
 ostringstream oss;
 if(s.read_individual_files){
  ...
  samples.resize(sample_end-sample_start+1);
  ...
  while(!ifs.eof()){
   ...
   if(!ifs.fail()){
    ...
    clog << "bgl = " << bgl << endl;
    Sample s;
    s.start_concentration = s.end_concentration = s.avg_concentration = bgl;
    Reading r;
    while(!iss.eof()){
     double photon_count;
     iss >> photon_count >> comma;
     r.features.raw_features.push_back(photon_count);
     if(r.features.raw_features.size() == 1){
      clog << "first feature = " << photon_count << endl;
     }
    }
    clog << "read " << r.features.raw_features.size() << endl;
